...
def load_parameters(path:str):
    ...
    with open(f"{path}", "rb") as the_file:
        ...
        nr_states, nr_actions = parameters["NS"], parameters["max_controls"]
        nr_stars = parameters["number_stars"]

        return max_fuel, nr_states, nr_actions, nr_stars

def load_star_values(path:str) -> (np.array, np.array):
    '''
    '''
    star_values, star_indices, star_indptr, star_shape = load_sparse_matrix(f"{path}", "star_graph")

    star_graph = to_sparse_matrix(*load_sparse_matrix(path, "star_graph"))
    stars = np.load(f"{path}/stars.npy")
    star_types = np.load(f"{path}/star_types.npy")

    return star_graph, stars, star_types
